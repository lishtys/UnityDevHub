...
public class SpawnerSystem_SpawnAndRemove : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        Entities
            ...
            .ForEach((Entity entity, int entityInQueryIndex, in SpawnData spawner, in LocalToWorld location) =>
        {
            ...
            for (var x = 0; x < spawner.CountX; x++)
            {
                for (var y = 0; y < spawner.CountY; y++)
                {
                    ...
                    commandBuffer.SetComponent(entityInQueryIndex, instance, new LifetimeData() { LifeTime = random.NextFloat(10.0F, 100.0F) });
                     commandBuffer.SetComponent(entityInQueryIndex, instance, new RotationSpeedData() { Degree = random.NextFloat(25.0F, 90.0F) });
                }
            }
